...
def packData(data):
    ...
    acc_scale = 4*9.8/32768.0               # acc measure range is 4g
    gyr_scale = 2*np.pi*250/(360*32768.0)   # gyroscope measurement range is 250 degree
...
# calculate knee angle using optical data
# datafile: data file path
# method: openCV or kinovea
# return measurement time and knee angle in list datastructures
def knee_angle_optical(datafile, method = 'openCV'):
    if method == 'openCV':
        frametime=1/59.9
        opencv_data=np.loadtxt(datafile)
        ...
        tptime, tp1x, tp1y, tp2x, tp2y, tp3x, tp3y = readKinoveaData(datafile)
    else:
        print('Invalid optical method')
