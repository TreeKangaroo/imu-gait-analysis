...
def make_model(input_shape=(numf, 10, 1)):
    inputs = keras.layers.Input(shape=input_shape, name='main_input')
    x = keras.layers.Conv2D(32, (numf, 3), padding="valid", strides=1,
                            kernel_initializer="he_normal", 
                            kernel_regularizer=l2(0.0005),
                            activation=actfunc)(inputs)
    x = keras.layers.Flatten()(x)
    x = keras.layers.Dense(10, kernel_initializer="he_normal",
...
def train_model():
    ...
    x_test = np.delete(x_test, [1, 2, 3, 4, 5, 6, 7, 8, 9], axis=1)
    x_train = np.delete(x_train, [1, 2, 3, 4, 5, 6, 7, 8, 9], axis=1)
